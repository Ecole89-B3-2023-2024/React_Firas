...
export default function Starships() {
    ...
    const [url, setUrl] = useState(`https://swapi.dev/api/starships/${id}`);
    ...
    function handleSubmit(e){
        e.preventDefault();
        setUrl(`https://swapi.dev/api/starships/${id}`);
    }
    if(!loaded){
        ...
        return (
            <div id="starships.list" className="row">
                ...
                <div className="col-4 d-flex flex-column mx-auto p-5 d-block border rounded bg-primary-subtle">
                    ...
                    <span className="fw-bold"> {starships.title}</span>
                    </h3>
                    <p className="text-center">Id de l'épisode : 
                    <span className="fw-bold"> {starships.episode_id}</span>
                    </p>
                    <p className="text-center">Exploration d'ouverture :
                    <span className="fw-bold"> {starships.opening_crawl}</span>
                    </p>
                    <p className="text-center">Directeur :
                    <span className="fw-bold"> {starships.director}</span>
                    </p>
                    <p className="text-center">Producteur :
                    <span className="fw-bold"> {starships.producer}</span>
                    </p>
                    <p className="text-center">Date de sortie :
                    <span className="fw-bold"> {starships.release_date}</span>
                    </p>
                </div>
